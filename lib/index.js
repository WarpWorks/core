...
class WarpWorks {
    ...
    getDir(name, file) {
        return utils.getDir(config, name, file);
    }
    ...
    useDB(dbName, nextFunction) {
        if (this.mongoDBs[dbName]) {
            ...
            try {
                mongoClient.connect(utils.getMongoURL(config, dbName), function(err, db) {
                    if (err) {
    ...
    removeAllDataFromDB(dbName) {
        ...
        this.useDB(dbName, persistence.removeAllDataFromDB);
    }
    ...
    applyTemplateFile(fn, domains) {
        ...
        return tokenSeq.map((token) => token.value).join('');
    }
