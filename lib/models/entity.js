...
class Entity extends Base {
	...
	getRelationshipByChildName(name){		
		...
		for (var rel in relationships){
			...
			{
				return getRelationshipByChildName(relentity.parentClass[0].name);
			}
	...
	getChildInstances(persistence,id) {
        return Promise.resolve(this.getDocuments(persistence, {parentID: id}, true));
    }
	...
	getRelationshipByName(relationshipName) {
        const relationships = this.getRelationships().filter((rel) => rel.name.toLowerCase() === relationshipName.toLowerCase());
