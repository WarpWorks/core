...
describe("lib/models/base", () => {
    ...
    describe("new Base()", () => {
        ...
        describe(".getWarpWorks()", () => {
            it("should return warpworks", () => {
                const EXPECTED_OUTPUT = {key: 'this is the expected object'};

                const type = 'someType';
                const parent = {
                    getWarpWorks() { return EXPECTED_OUTPUT; }
                };
                const id = 123;
                const name = 'some name with space';
                const desc = 'some description';
                ...
                const base = new Base(type, parent, id,name, desc);
                const warpworks = base.getWarpWorks();

                expect(warpworks).to.deep.equal(EXPECTED_OUTPUT);
            });

            it("should define warpWorks for the parent", () => {
                const type = 'someType';
                const parent = new WarpWorks();
                const id = 123;
                const name = 'some name with space';
                const desc = 'some description';
                ...
                expect(warpworks).to.have.property('foo').to.equal('bar');
            });
        });
